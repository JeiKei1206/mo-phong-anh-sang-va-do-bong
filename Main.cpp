...
int init() {
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "sv102240088Khoa-sv102240118Trung-sv102240113Tien", nullptr, nullptr);
	if (nullptr == window) {
...
int main()
{
	...
	GLuint shader = loadSHADER("DATA/vertex.shader", "DATA/fragment.shader");
	glUseProgram(shader);

	MeshData pokePhong = setupPhongMesh("DATA/pokeball.obj");
	MeshData pokeGouraud = setupGouraudMesh("DATA/pokeball.obj");
	MeshData heartPhong = setupPhongMesh("DATA/heart.obj");
	MeshData heartGouraud = setupGouraudMesh("DATA/heart.obj");
	MeshData teapotPhong = setupPhongMesh("DATA/teapot.obj");
	MeshData teapotGouraud = setupGouraudMesh("DATA/teapot.obj");
